...
dev_docker_setup_windows:
	@cd dev && setup_db.sh $(filter-out $@,$(MAKECMDGOALS))

dev_docker_setup_linux:
	@cd dev && chmod +x setup_db.sh && dos2unix setup_db.sh && bash setup_db.sh $(filter-out $@,$(MAKECMDGOALS))

dev_docker_manage: # execute manage.py commands
	@cd dev && docker-compose run app /code/dbaas/manage.py $(filter-out $@,$(MAKECMDGOALS))

dev_docker_migrate:
	@cd dev && docker-compose run app python /code/dbaas/manage.py migrate
